...
public class AES {/*
  ...
  /**
     ...
     * @param plaintext The plaintext (UTF-8)
     * @param key A BigInteger key. Size doesn't matter.
     * @return The encoded bytes
     */
    public static byte[] encode(String plaintext, BigInteger key) {
...
/**
     ...
     * @param key The same key used to encode
     * @return The plaintext (UTF-8)
     */
    public static String decode(byte[] ciphertext, BigInteger key) {
        byte[] thing = key.toByteArray();
        Random st = new Random(thing[0]);
        for (int i = 0; i < thing.length; i++) {
            st.nextInt(130 + thing[i]);
    ...
    private static byte[] encode1(char[] password, byte[] salt, String plaintext) {
        try {
    ...
    private static String decode1(char[] password, byte[] ciphertet, byte[] salt) {
        try {
