	...
	public void paintComponent(Graphics g){
		...
		Main.maxCol=100;
		 ...
		 if (unbounded1){
			 ...
			 Main.maxRow=Main.current.size()==0?Main.antY:Main.current.get(0).Row;
			 Main.maxCol=Main.current.size()==0?Main.antX:Main.current.get(0).Col;
			 //Main.minRow=Main.maxRow;
			 //Main.minCol=Main.maxCol;
			 if (Main.antY<Main.minRow){
				 Main.minRow=Main.antY;
			 }
			 if (Main.antY>Main.maxRow){
				 Main.maxRow=Main.antY;
			 }
			 if (Main.antX<Main.minCol){
				 Main.minCol=Main.antX;
			 }
			 if (Main.antX>Main.maxCol){
				 Main.maxCol=Main.antX;
			 }
		...
		for (int row=Main.minRow; row<=Main.maxRow; row++){
			...
			//Systeout.println(q);
				...
				g.fillRect(C,R,Main.S,Main.S);
        		 ...
        		 int R=Main.S*(Main.antY-Main.minRow) +80;
        			int C=Main.S*(Main.antX-Main.minCol) +80;
        				g.fillRect(C,R,Main.S,Main.S);
